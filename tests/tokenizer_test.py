...
def test_tokenizer_basics() -> None:
    assert tokenize("if  3\nwhile ") == [
        ...
        Token("3", TokenType.INT_LITERAL, SourceLocation(any=True)),
...
def test_operators() -> None:
    assert tokenize("2 + 2") == [
        Token("2", TokenType.INT_LITERAL, SourceLocation(any=True)),
...
def test_punctuation() -> None:
    ...
    assert tokenize("\n{25};") == [
        Token("{", TokenType.PUNCTUATION, SourceLocation(any=True)),
        Token("25", TokenType.INT_LITERAL, SourceLocation(any=True)),
        Token("}", TokenType.PUNCTUATION, SourceLocation(any=True)),
        Token(";", TokenType.PUNCTUATION, SourceLocation(any=True)),
    ]
